    ...
    public UpdateContentSchemaCommandValidator()
    {
        ...
        RuleForEach(x => x.Fields).ChildRules(field =>
        {
            ...
            field.RuleFor(f => f.Slug)
                ...
                .Matches("^[a-z0-9]+(?:[-_][a-z0-9]+)*$").WithMessage("Field slug must use lowercase letters, numbers, hyphen or underscore.")
                .Must(slug => !ReservedFieldSlugs.Contains(slug))
                .WithMessage("Field slug is reserved by native content fields.");
