...
public sealed class BlockNoteMarkdownFieldValidator : IFieldValidator
{
    ...
    private static void ValidateEmbedHosts(FieldDefinition fieldDef, JsonElement root, List<SchemaValidationError> errors)
    {
        ...
        foreach (var embed in embeds.EnumerateArray())
        {
            ...
            // Internal MediaManager URLs are generated by the CMS and should not be blocked
            // by third-party embed host allow-lists.
            if (IsInternalMediaFileUrl(url))
                continue;
    ...
    private static bool IsInternalMediaFileUrl(string url)
    {
        ...
        // Expected: /api/projects/{projectId}/media/{mediaId}/file
        ...
        if (!Guid.TryParse(segments[2], out _))
            return false;
        if (!segments[3].Equals("media", StringComparison.OrdinalIgnoreCase))
            return false;
        if (!Guid.TryParse(segments[4], out _))
            return false;
        if (!segments[5].Equals("file", StringComparison.OrdinalIgnoreCase))
            return false;
