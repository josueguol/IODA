...
export function EditContentPage() {
  ...
  const contentId = paramContentId ?? searchParams.get('contentId') ?? undefined
  ...
  if (!contentId) {
    return (
      <div className="edit-content-page">
        <p className="edit-content-page__error">Falta el identificador del contenido.</p>
  ...
  return (
    <div className="edit-content-page">
      <div className="edit-content-page__header">
        <h1 className="edit-content-page__page-title">Editar contenido</h1>
        <Link to="/content" className="edit-content-page__back-link">
          Volver al listado
        </Link>
      </div>
      ...
      <ContentEditorSplitLayout
        left={
          <>
            <h2 className="edit-content-page__section-title">Campos del contenido</h2>
            <label htmlFor="edit-title" className="edit-content-page__title-label">
            ...
            />
            <label htmlFor="edit-slug" className="edit-content-page__title-label">
              Slug (URI) *
            </label>
            <input
              id="edit-slug"
        ...
        right={
          <>
            <h2 className="edit-content-page__section-title">Propiedades</h2>

            <div className="edit-content-page__status">Estado: {content.status} · v{content.currentVersion}</div>

            <div className="edit-content-page__meta">
              Creado: {new Date(content.createdAt).toLocaleString()}
              {content.updatedAt != null && <> · Actualizado: {new Date(content.updatedAt).toLocaleString()}</>}
              {content.createdBy && <> · Creado por: {content.createdBy}</>}
              {content.updatedBy != null && content.updatedBy !== '' && <> · Actualizado por: {content.updatedBy}</>}
            </div>
            <div className="edit-content-page__meta">
              ...
              <>
                ...
                {editHierarchyIds.length > 0 && (
                  <div className="edit-content-page__order-wrap">
                    <label htmlFor="edit-primary-section" className="edit-content-page__order-label">
                      ...
                      return (
                        <div key={siteId}>
                          ...
                          <input
                            ...
                            type="text"
                            className="edit-content-page__order-input edit-content-page__order-input--full"
            ...
            {content.status === 'Published' && content.publishedAt && (
              <div className="edit-content-page__section edit-content-page__section--reindex">
                ...
                <p>Si el contenido no aparece en la búsqueda, puedes reindexarlo manualmente.</p>
                {reindexMessage && (
            ...
            <div className="edit-content-page__versions">
              <div className="edit-content-page__versions-header">
                <h3 className="edit-content-page__versions-title">Historial de versiones (v{content.currentVersion})</h3>
                <button
                  type="button"
                  ...
                  {!versionsLoading && versions.length > 0 && (
                    <div className="edit-content-page__versions-stack">
                      {versions.map((v) => {
                        ...
                        const statusClass =
                          v.status === 'Published'
                            ? 'edit-content-page__version-status--published'
                            : v.status === 'Draft'
                              ? 'edit-content-page__version-status--draft'
                              : 'edit-content-page__version-status--pending'
                        return (
                          <div key={v.id} className={`edit-content-page__version-card ${isCurrent ? 'edit-content-page__version-card--current' : ''}`}>
                            <div className="edit-content-page__version-row">
                              <div>
                                ...
                                <span className="edit-content-page__version-title">{v.title}</span>
                              </div>
                              <div className="edit-content-page__version-meta">
                                <span className={`edit-content-page__version-status ${statusClass}`}>{v.status}</span>
                                <span className="edit-content-page__version-date">{new Date(v.createdAt).toLocaleString()}</span>
                              </div>
                            </div>
                            {v.comment && <p className="edit-content-page__version-comment">{v.comment}</p>}
